...
import { isAdmin, ALL_MENUS } from '@/app/lib/auth'
import { hashPassword, verifyPassword, needsUpgrade } from '@/app/lib/password'
...
export async function POST(req: NextRequest) {
  ...
  // 레거시 SHA-256 해시는 로그인에 성공하는 순간 scrypt 로 갈아끼운다 (무중단 마이그레이션)
  if (needsUpgrade(user.password)) {
    await supabase.from('haru_users').update({ password: hashPassword(password) }).eq('id', user.id)
  }
